  ...
  <div class="payment-success">
    ...
    <div v-if="order" class="summary">
      ...
      <ul>
        ...
        <li><strong>Montant :</strong> {{ (order.amount_total / 100).toFixed(2) }} €</li>
        <li>
        ...
        <li><strong>ID Session :</strong> {{ sessionId }}</li>
        <li><strong>ID PaymentIntent :</strong> {{ order.payment_intent }}</li>
      </ul>

      <RouterLink to="/profil/commandes" class="btn">Voir mes commandes</RouterLink>
    </div>

    <!-- ❌ Erreur (affichée uniquement si on a fini le chargement ET rien trouvé) -->
    <div v-else-if="hasLoaded" class="error">
