export type BusEvent<T extends string = string, F = void> = T extends '*'
  ? never
  : F extends void
    ? { name: T }
    : { name: T; arg: F }
...
export type EventBusCallBack<
  B extends BusEvent,
  Name extends BusEventName<B> | '*' = BusEventName<B> | '*',
> = Name extends '*' ? (event: B) => void : (event: Extract<B, { name: Name }>) => void
