...
--        - Vues sessions_stats, sessions_by_day, sessions_by_country
--        - RPC track_session, update_session_activity, end_session
...
COMMENT ON COLUMN public.profiles.login_count IS 'Nombre total de connexions';
...
-- ============================================================
-- BLOC — USER SESSIONS TRACKING (V6.2)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.user_sessions (
  ...
  -- Identifiant utilisateur (null = visiteur anonyme)
  user_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,

  -- Identifiant de session anonyme (pour visiteurs non connectés)
...
COMMENT ON TABLE public.user_sessions IS 'Tracking des sessions utilisateurs (authentifiés et anonymes)';
COMMENT ON COLUMN public.user_sessions.session_id IS 'Identifiant unique de session (UUID généré côté client)';
COMMENT ON COLUMN public.user_sessions.session_type IS 'authenticated = connecté, anonymous = visiteur, guest_checkout = commande invité';
...
-- Insertion publique (pour tracking anonyme)
CREATE POLICY "Public insert sessions" ON public.user_sessions
  FOR INSERT TO anon, authenticated
  WITH CHECK (true);

-- Mise à jour publique (pour update last_activity)
...
-- Grant access aux vues pour les admins
...
BEGIN
  -- Vérifie si une session existe déjà pour ce session_id (dans les 24h)
  ...
  -- Met à jour les stats du profil si user connecté
  IF p_user_id IS NOT NULL THEN
